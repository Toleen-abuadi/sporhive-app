...
export function ResetPasswordScreen() {
  ...
  const startResendTimer = () => setResendIn(45);
  ...
  const requestOtp = useCallback(async () => {
    ...
    const payload =
      mode === MODES.PUBLIC
        ? { user_kind: 'public', phone: phoneValue.e164 }
        : {
            user_kind: 'player',
            academy_id: Number(academy?.id),
            username: username.trim(),
            phone_number: phoneValue.e164,
          };
  ...
  }, [academy, mode, phoneValue, setLastSelectedAcademyId, t, username, toast]);
  ...
  const copyGenerated = async () => {
    ...
    try {
      ...
      if (__DEV__) console.warn('Clipboard copy failed', error);
    }
              ...
              <View style={styles.form}>
                ...
                {mode === MODES.PLAYER ? (
                  <>
                    ...
                    />
                    {errors.academy ? (
                      <Text variant="caption" color={colors.error} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                        {errors.academy}
                      </Text>
                    ) : null}
            ...
            {step === STEPS.OTP && (
              <View style={styles.form}>
                ...
                <View style={[styles.resendRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                  ...
                  <Pressable onPress={() => resendIn === 0 && requestOtp()} disabled={resendIn > 0}>
                    <Text
                      variant="caption"
                      weight="bold"
            ...
            {((step === STEPS.FINAL && mode === MODES.PLAYER) || complete) && (
              <View style={styles.finalCard}>
                ...
                <Text variant="h3" weight="bold" style={styles.centered}>
                  {mode === MODES.PLAYER ? t('auth.reset.playerFinalTitle') : t('auth.reset.publicFinalTitle')}
                </Text>
                <Text variant="bodySmall" color={colors.textSecondary} style={styles.centered}>
                  {mode === MODES.PLAYER ? t('auth.reset.playerFinalSubtitle') : t('auth.reset.publicFinalSubtitle')}
                </Text>

                {mode === MODES.PLAYER ? (
                  <View style={[styles.passwordCard, { borderColor: colors.border }]}>
                    <Text variant="caption" color={colors.textMuted} style={{ textAlign: isRTL ? 'right' : 'left' }}>
                    ...
                    <View style={[styles.passwordActions, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
...
const styles = StyleSheet.create({
  ...
  resendRow: { justifyContent: 'space-between', alignItems: 'center' },

  cta: { marginTop: spacing.sm, borderRadius: borderRadius.pill },

  finalCard: { alignItems: 'center', gap: spacing.md },
  finalIcon: { width: 68, height: 68, borderRadius: 34, alignItems: 'center', justifyContent: 'center' },
  ...
  passwordActions: { justifyContent: 'space-between' },
