...
export function PortalOrderDetailScreen() {
  ...
  const order = useMemo(() => {
    ...
    return (orders || []).find(
      (item) => String(item?.id || item?.reference) === String(target)
    );
  }, [orderId, orders]);
  ...
  if (!order) {
    return (
      <AppScreen safe>
        ...
        <EmptyState
          title={t('portal.orders.detailMissingTitle')}
          message={t('portal.orders.detailMissingDescription')}
      ...
      <AppScreen safe scroll>
        ...
        <Card style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text variant="body" weight="bold" color={colors.textPrimary}>
            {order?.title || t('portal.orders.defaultTitle')}
          </Text>
          <Text variant="caption" color={colors.textSecondary}>
            {t('portal.orders.statusLabel', { status: order?.status || t('portal.orders.status.pending') })}
          </Text>
          <View style={styles.row}>
            <Text variant="caption" color={colors.textMuted}>
              {t('portal.orders.referenceLabel')}
            </Text>
            <Text variant="bodySmall" color={colors.textPrimary}>
              {order?.reference || order?.id || t('portal.common.placeholder')}
            </Text>
          </View>
          <View style={styles.row}>
            <Text variant="caption" color={colors.textMuted}>
              {t('portal.orders.createdLabel')}
            </Text>
            <Text variant="bodySmall" color={colors.textPrimary}>
              {order?.created_at || t('portal.common.placeholder')}
            </Text>
          </View>
          <View style={styles.row}>
            <Text variant="caption" color={colors.textMuted}>
            ...
            <Text variant="bodySmall" color={colors.textPrimary}>
              {order?.total || order?.amount || t('portal.common.placeholder')}
            </Text>
          </View>
        </Card>
        ...
        {Array.isArray(order?.items) && order.items.length ? (
          <Card style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Text variant="body" weight="bold" color={colors.textPrimary}>
              {t('portal.orders.itemsTitle')}
            </Text>
            {order.items.map((item, idx) => (
              <View key={`${item?.id || idx}`} style={styles.itemRow}>
                <Text variant="bodySmall" color={colors.textPrimary}>
                  {item?.name || t('portal.orders.itemFallback')}
                </Text>
                <Text variant="caption" color={colors.textSecondary}>
                  {item?.size || item?.quantity ? `${item?.size || ''} ${item?.quantity || ''}`.trim() : t('portal.common.placeholder')}
                </Text>
              </View>
            ))}
          </Card>
